...
const Gradient = ({ conic, className, small, }) => {
...
const LinksSection = async () => {
    ...
    return (<div className={page_module_css_1.default.grid}>
      {links.map(({ title, url, description }) => (<card_1.Card className={page_module_css_1.default.card} href={url} key={title} title={title}>
          {description}
        </card_1.Card>))}
    </div>);
};
const LinksSectionForTest = () => {
    return (<div className={page_module_css_1.default.grid}>
      <card_1.Card className={page_module_css_1.default.card} href={'url'} title={'title'}>
        description
      </card_1.Card>
    </div>);
};
const RootPage = ({ params }) => {
    return (<main className={page_module_css_1.default.main}>
      <div className={page_module_css_1.default.description}>
        <p>
          examples/<code_1.Code className={page_module_css_1.default.code}>with-nestjs</code_1.Code>
        </p>
        <div>
      ...
      <button_1.Button appName="web (with-nestjs)" className={page_module_css_1.default.button}>
        Click me!
      </button_1.Button>
      ...
      {/**
         ...
         */}
      {params.forTest ? (<LinksSectionForTest />) : (<react_1.Suspense fallback={'Loading links...'}>{<LinksSection />}</react_1.Suspense>)}
    </main>);
};
